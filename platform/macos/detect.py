...
def get_opts():
    ...
    return [
        ...
        # Exclude display and audio unless explicitly enabled.
        BoolVariable("display", "Enable the window and input backend", False),
        BoolVariable("coreaudio", "Use CoreAudio", False),
        BoolVariable("coremidi", "Use CoreMIDI", False),
    ]
...
def get_flags():
    return {
        "arch": detect_arch(),
        "use_volk": False,
        "metal": False,  # Enable rendering only in display-capable builds.
...
def configure(env: "SConsEnvironment"):
    ...
    if env["display"]:
        env.Append(CPPDEFINES=["MACOS_DISPLAY_ENABLED"])
    if env["coreaudio"]:
        env.Append(CPPDEFINES=["COREAUDIO_ENABLED"])
    if env["coremidi"]:
        env.Append(CPPDEFINES=["COREMIDI_ENABLED"])
    env.Append(
        LINKFLAGS=[
            "-framework",
            "Cocoa",
