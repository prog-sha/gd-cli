...
def get_opts():
    ...
    return [
        ...
        # Build display, input, menus, and speech only when explicitly enabled.
        BoolVariable("display", "Enable the window and input backend", False),
...
def configure_msvc(env: "SConsEnvironment"):
    ...
    # Include audio and MIDI only in display-enabled builds.
    if env["display"]:
        env.AppendUnique(CPPDEFINES=["WASAPI_ENABLED", "WINMIDI_ENABLED"])
...
def configure_mingw(env: "SConsEnvironment"):
    ...
    # Keep optional-feature boundaries consistent across supported toolchains.
    if env["display"]:
        env.AppendUnique(CPPDEFINES=["WASAPI_ENABLED", "WINMIDI_ENABLED"])
    env.Append(
        CPPDEFINES=[
...
def configure(env: "SConsEnvironment"):
    ...
    if env.msvc:
        ...
        configure_mingw(env)
    if env["display"]:
        env.AppendUnique(CPPDEFINES=["WINDOWS_DISPLAY_ENABLED"])
