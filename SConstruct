...
opts.Add(
    EnumVariable(
        "target", "Compilation target", "template_debug", ["template_release", "template_debug"], ignorecase=2
    )
)
...
opts.Add(BoolVariable("vulkan", "Enable the Vulkan rendering driver", False))  # Keep rendering and input in the optional game layer.
opts.Add(BoolVariable("opengl3", "Enable the OpenGL/GLES3 rendering driver", False))  # Keep rendering and input in the optional game layer.
opts.Add(BoolVariable("d3d12", "Enable the Direct3D 12 rendering driver on supported platforms", False))
opts.Add(BoolVariable("metal", "Enable the Metal rendering driver on supported platforms (Apple arm64 only)", False))
opts.Add(BoolVariable("use_volk", "Use the volk library to load the Vulkan loader dynamically", True))
opts.Add(BoolVariable("accesskit", "Enable the AccessKit driver for screen reader support", False))  # Keep display support in the optional game layer.
opts.Add(BoolVariable("angle", "Enable the ANGLE rendering driver for OpenGL ES 3.0 on supported platforms", True))
opts.Add(BoolVariable("sdl", "Enable the SDL3 input driver", False))  # Keep rendering and input in the optional game layer.
opts.Add(
    EnumVariable(
...
opts.Add(
    BoolVariable(
        "dev_mode", "Alias for dev options: verbose=yes warnings=extra werror=yes strict_checks=yes", False
    )
)
...
opts.Add(BoolVariable("disable_3d", "Disable 3D nodes for a smaller executable", True))  # Build only in the optional game layer.
opts.Add(BoolVariable("disable_advanced_gui", "Disable advanced GUI nodes and behaviors", True))  # Build only in the optional game layer.
opts.Add(BoolVariable("disable_physics_2d", "Disable 2D physics nodes and server", True))  # Build only in the optional game layer.
opts.Add(BoolVariable("disable_physics_3d", "Disable 3D physics nodes and server", True))  # Build only in the optional game layer.
...
# Enable only the modules required by the headless runtime by default.
# Leave image, audio, rendering, physics, and 3D modules in the optional game layer.
# Include the remaining modules when building that layer.
GD_PRIME_MODULES = {
    "gdscript",  # Script execution.
    "enet",  # Multiplayer transport.
    "multiplayer",  # Remote calls and synchronization.
    "websocket",  # Stream messaging.
    "webrtc",  # Peer transport.
    "upnp",  # Port mapping.
    "mbedtls",  # TLS and cryptographic keys.
    "regex",  # Pattern matching.
    "zip",  # Archive access.
    "jsonrpc",  # Remote request handling.
}
